...
;
  ...
  If OpenDatabase(#DB_Id, "host=localhost port=3306 dbname="+ DB_name$, DB_usuario$, DB_password$)
     ...
     ;Primer usuario que se nserta en la tabla
     DB_Command$ ="INSERT INTO usuarios (nombre, correo, oficina) VALUES ('Juan', 'juan@gmail.com', 'administracion')"
     If DatabaseUpdate(#DB_Id, DB_Command$)
        MessageRequester("MySQL", "Juan insertado en la tabla")  
     Else   
        MessageRequester("MySQL", "Juan ya existe en la tabla")  
     EndIf
     
     ;Segundo usuario que se nserta en la tabla
     DB_Command$ ="INSERT INTO usuarios (nombre, correo, oficina) VALUES ('Pedro', 'pedro@yahoo.com', 'ventas')"
     If DatabaseUpdate(#DB_Id, DB_Command$)
        MessageRequester("MySQL", "Pedro insertado en la tabla")  
     Else   
        MessageRequester("MySQL", "Pedro ya existe en la tabla")  
     EndIf
     
     ;Tercer usuario que se nserta en la tabla
     DB_Command$ ="INSERT INTO usuarios (nombre, correo, oficina) VALUES ('Antonio', 'antonio@live.com', 'expediciones')"
     If DatabaseUpdate(#DB_Id, DB_Command$)
        MessageRequester("MySQL", "Antonio insertado en la tabla")  
     Else   
        MessageRequester("MySQL", "Antonio ya existe en la tabla")  
     EndIf
     
     ;Cuarto usuario que se nserta en la tabla
